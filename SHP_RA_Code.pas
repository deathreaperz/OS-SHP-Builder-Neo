...
uses BasicDataTypes, Dialogs, SysUtils;
...
{$ELSE}
   push rsi
   push rdi
   mov    rax, SourceEnd     // Prepare SourceEnd
   mov    rbx, Source        // Prepare Source
   xor    rcx, rcx           // Who is ecx? Gets 0.
   mov    rdi, p             // Prepare p.
   mov[rdi], rcx             // set p[0] = 0;
   dec    rbx
   @@next_s:   inc    rbx
   // This part is a loop. If Source doesnt reach 'r', go to @@next, otherwise, say bye bye.
   // Edx gets XORed at the process.
   xor    rdx, rdx   // EDX gets 0.
   mov    rsi, r
   mov    rdi, rbx
   cmp    rdi, rsi
   jnb    @@end
   @@next:     inc    rdx   // EDX increases. (seems like a counter)
   cmp    rsi, rax
   jnb    @@end_line // also a loop
   cmpsb
   je    @@next
  ...
  4 copy 11111111 c c p p
   ...
   begin
      ...
      begin
         ...
         {$IFDEF Win32}
         copyp := PByte(integer(integer(writep) - (((code and $f) shl 8) + readp^)));
         {$ELSE}
         copyp := PByte(int64(int64(writep) - (((code and $f) shl 8) + readp^)));
         {$ENDIF}
         Inc(readp);
         while (Counter <> 0) do
         begin
