...
uses FormMain, SysUtils, Dialogs, Menus, BasicDataTypes;
         ...
         case (token) of
            ...
            begin
               ...
               begin
                  ...
                  CatName[position] := Char(token);
               end
               else if OnUrl then
               begin
                  SetLength(FrmMain.SiteList[counter].SiteUrl, position);
                  FrmMain.SiteList[counter].SiteUrl[position] := Char(token);
               end
               else if OnName then
               begin
                  SetLength(FrmMain.SiteList[counter].SiteName, position);
                  FrmMain.SiteList[counter].SiteName[position] := Char(token);
               end;
               Inc(position);
            end;
         end;
