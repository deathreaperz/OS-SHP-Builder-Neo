...
uses
   ...
   Dialogs, ExtCtrls, ComCtrls, StdCtrls, Spin, BasicDataTypes, SHP_file, SHP_Shadows,
...
//    procedure lbl_total_framesDblClick(Sender: TObject);
   ...
   private
      ...
      procedure DrawGrid;
      procedure DrawGridPixel(const _x, _y: integer; const _epsilon: real);
   public
      { Public declarations }
...
uses FormMain, FormPreview, OS_SHP_Tools, SHP_Engine, SHP_DataMatrix;
...
procedure TFrmSHPImage.DrawGridPixel(const _x, _y: integer; const _epsilon: real);
var
   c,xZ,yZ: integer;
   alpha: real;
   SHPData : TSHPImageData;
begin
   SHPData := Data;
   // check x = y line
   alpha := (_y - (_x/2) + (SHPData^.SHP.Header.Width / 4) - (SHPData^.SHP.Header.Height / 2) - 1 - GridSize) / GridSize;
   // Check if alpha is an integer. If it is, paint the grid. Otherwise, doesn't paint.
   if (abs(alpha - trunc(alpha)) < _epsilon) then
   begin
      xZ:=(_x+1)*Zoom;
      yZ:=(_y-2)*Zoom;
   ...
   begin
      // check x = -y line
      alpha := ((SHPData^.SHP.Header.Width / 4) + (SHPData^.SHP.Header.Height / 2) - _y - 1 - (_x/2) - GridSize) / GridSize;
      // Check if alpha is an integer. If it is, paint the grid. Otherwise, doesn't paint.
      if (abs(alpha - trunc(alpha)) < _epsilon) then
      begin
         xZ:=(_x+1)*Zoom;
...
begin
   SHPData := Data;
   // 3.38 - the math way
   // Let's do some easier coding here.
   if not FullGrid then
   begin
      if GridEpsilon > 0.01 then
      begin
         c := 1;
      end
      else
      ...
      for y := (SHPData^.SHP.Header.Height div 2) to SHPData^.SHP.Header.Height - 1 do
      begin
         for x := max(0, (SHPData^.SHP.Header.Width div 2) - c) to min((SHPData^.SHP.Header.Width div 2) + c, SHPData^.SHP.Header.Width - 1) do
         begin
            DrawGridPixel(x, y, GridEpsilon);
         end;
         inc(c, 2);
      end;
   end
...
begin
   ...
   // Grid update.
   FrmMain.UncheckGrids();
   if ShowGrid = true then
   begin
      if GridSize = 30 then
      begin
         FrmMain.TbShowGrid.ImageIndex := 2;
         FrmMain.RedAlert2GridsView.Checked := true;
      end
      else if GridSize = 24 then
      begin
         FrmMain.TbShowGrid.ImageIndex := 0;
         FrmMain.TiberianSunGridsView.Checked := true;
      end
      else
      begin
         FrmMain.TbShowGrid.ImageIndex := 22;
         FrmMain.NoneGridsView.Checked := true;
      end;
   end
   else
   begin
   ...
   FrmMain.UncheckFullGrid();
   if FullGrid = true then
   begin
      FrmMain.ShowFullGrid1.Checked := true;
   end
   else
   begin
      FrmMain.ShowUsefulGridsOnly1.Checked := true;
   end;
   FrmMain.UncheckLineGrid();
   if GridEpsilon = 0.01 then
   begin
      FrmMain.ShowGridNarrowLines1.Checked := true;
   end
   else
   begin
      FrmMain.ShowGridStrongerLines1.Checked := true;
   end;
...
begin
   ...
   FrmMain.UncheckFullGrid();
   FrmMain.ShowUsefulGridsOnly1.Checked := true;
   if (ShowGrid = true) and (GridSize = 30) then
   begin
      GridEpsilon := 0.02;
   end
   else
