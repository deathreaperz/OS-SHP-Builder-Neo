...
uses WindowsUtils, SysUtils, FormMain, FormPreferences, BasicDataTypes;
...
begin
   ...
   begin
      ...
      if FrmMain.MaxOpenFiles > 0 then
         for count := 0 to (FrmMain.MaxOpenFiles-1) do
            BlockRead(F,FrmMain.OpenFilesList[count],255 * Sizeof(char8));
      BlockRead(F,FrmMain.OpenDir,255 * Sizeof(char8));
      BlockRead(F,FrmMain.SaveDir,255 * Sizeof(char8));
      BlockRead(F,FrmMain.ImportDir,255 * Sizeof(char8));
      BlockRead(F,FrmMain.ExportDir,255 * Sizeof(char8));
      BlockRead(F,FrmMain.PreviewBrush,Sizeof(Boolean));
      BlockRead(F,FrmMain.OtherOptionsData.AutoSelectSHPType,Sizeof(Boolean));
      FrmMain.OtherOptionsData.LastPalettePath := '';
      FrmMain.OtherOptionsData.BackgroundEnabled := true;
