...
uses
  ...
  Dialogs, StdCtrls, ExtCtrls, Menus, Buttons, BasicDataTypes, shp_file,shp_engine,
  palette, Shp_Engine_Image, Spin, math, ComCtrls,clipbrd, ImgList, ToolWin,Mouse,
  ...
  PaletteControl, System.ImageList;

Const
   SHP_BUILDER_VER = '3.38';
{$IFDEF _BETA}
   SHP_BUILDER_BETA_VER = '34';
   SHP_BUILDER_INTERNAL_VERSION = '3.37.99.034';
{$ENDIF}
...
type
   ...
   TSHPBuilderFrmMain = class(TForm)
    ...
    ShowGridsView: TMenuItem;
    NoneGridsView: TMenuItem;
    TiberianSunGridsView: TMenuItem;
    RedAlert2GridsView: TMenuItem;
    N24: TMenuItem;
    ShowUsefulGridsOnly1: TMenuItem;
    ShowFullGrid1: TMenuItem;
    N25: TMenuItem;
    ShowGridStrongerLines1: TMenuItem;
    ShowGridNarrowLines1: TMenuItem;
...
//      Procedure WorkOutImageClick(var SHP: TSHP; var X,Y : integer; var OutOfRange : boolean; zoom:byte);
    ...
    procedure ShowUsefulGridsOnly1Click(Sender: TObject);
    procedure ShowFullGrid1Click(Sender: TObject);
    procedure ShowGridStrongerLines1Click(Sender: TObject);
    procedure ShowGridNarrowLines1Click(Sender: TObject);
   private
...
//      PaletteSchemes_No : Integer;
      ...
      procedure UncheckGrids();
      procedure UncheckFullGrid();
      procedure UncheckLineGrid();
   end;
         ...
         if Idx < ActiveData^.PaletteMax then
            ...
            begin
               ...
               begin // the current pen
                  ...
                  mixcol := min(red + green + blue, 255);
                  Pen.Color := RGB(min(128 + mixcol, 255),max(255 - mixcol, 0), mixcol);
                  //Pen.Mode := pmNotXOR;
...
procedure TSHPBuilderFrmMain.UncheckLineGrid();
begin
   ShowGridNarrowLines1.Checked := false;
   ShowGridStrongerLines1.Checked := false;
end;

procedure TSHPBuilderFrmMain.ShowGridNarrowLines1Click(Sender: TObject);
begin
   UncheckLineGrid();
   ShowGridNarrowLines1.Checked := true;
   ActiveForm^.GridEpsilon := 0.01;
   ActiveForm^.RefreshImage1;
end;

procedure TSHPBuilderFrmMain.ShowGridStrongerLines1Click(Sender: TObject);
begin
   UncheckLineGrid();
   ShowGridStrongerLines1.Checked := true;
   if ActiveForm^.GridSize = 30 then
   begin
      ActiveForm^.GridEpsilon := 0.02;
   end
   else
   begin
      ActiveForm^.GridEpsilon := 0.03;
   end;
   ActiveForm^.RefreshImage1;
end;

procedure TSHPBuilderFrmMain.UncheckFullGrid();
begin
   ShowFullGrid1.Checked := false;
   ShowUsefulGridsOnly1.Checked := false;
end;

procedure TSHPBuilderFrmMain.ShowFullGrid1Click(Sender: TObject);
begin
   UncheckFullGrid();
   ShowFullGrid1.Checked := true;
   ActiveForm^.FullGrid := true;
   ActiveForm^.RefreshImage1;
end;

procedure TSHPBuilderFrmMain.ShowUsefulGridsOnly1Click(Sender: TObject);
begin
   UncheckFullGrid();
   ShowUsefulGridsOnly1.Checked := true;
   ActiveForm^.FullGrid := false;
   ActiveForm^.RefreshImage1;
end;

procedure TSHPBuilderFrmMain.UncheckGrids();
begin
   NoneGridsView.Checked := false;
   TiberianSunGridsView.Checked := false;
   RedAlert2GridsView.Checked := false;
end;
...
begin
   ...
   if ActiveForm^.GridEpsilon = 0.02 then
   begin
      ActiveForm^.GridEpsilon := 0.03;
   end;
   UncheckGrids();
   TiberianSunGridsView.Checked := true;
   ActiveForm^.RefreshImage1;
end;
...
begin
   ...
   if ActiveForm^.GridEpsilon = 0.03 then
   begin
      ActiveForm^.GridEpsilon := 0.02;
   end;
   UncheckGrids();
   RedAlert2GridsView.Checked := true;
   ActiveForm^.RefreshImage1;
end;
